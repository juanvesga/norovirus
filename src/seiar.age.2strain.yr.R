...
update(M[]) <- if ( ((step %% 365) == 0) && i > 1 ) 
  M[i-1] - n_MS[i-1] - n_muM[i-1]  else
  M[i] + n_bM[i] - n_MS[i] - n_muM[i] 
...
update(S[]) <- if ( ((step %% 365) == 0) && i > 1 )
  S[i-1]  + n_MS[i-1] + n_Rj_S[i-1] + n_Rjk_S[i-1] + n_Rk_S[i-1] - 
  n_S_Ej[i-1] - n_S_Ek[i-1] - n_muS[i-1]  else
  S[i] + n_bS[i] + n_MS[i] + n_Rj_S[i] + n_Rjk_S[i] + n_Rk_S[i] - 
  n_S_Ej[i] - n_S_Ek[i] - n_muS[i]  

# Strain 1infected(GII.4)
update(Ej[]) <- if ( ((step %% 365) == 0) && i > 1 )
  Ej[i-1] + n_S_Ej[i-1] - n_Ej_Ij[i-1] - n_muEj[i-1]  else
  Ej[i] + n_S_Ej[i] - n_Ej_Ij[i] - n_muEj[i]

update(Ij[]) <- if ( ((step %% 365) == 0) && i > 1 )
  Ij[i-1] + n_Ej_Ij[i-1] - n_Ij_Aj[i-1] - n_muIj[i-1] else
  Ij[i] + n_Ej_Ij[i] - n_Ij_Aj[i] - n_muIj[i] 

update(Aj[]) <- if ( ((step %% 365) == 0) && i > 1 )
  Aj[i-1] + n_Ij_Aj[i-1] + n_Rj_Aj[i-1] - n_Aj_Rj[i-1]  - n_muAj[i-1] else
  Aj[i] + n_Ij_Aj[i] + n_Rj_Aj[i] - n_Aj_Rj[i]  - n_muAj[i] 

update(Rj[]) <- if ( ((step %% 365) == 0) && i > 1 )
  Rj[i-1] + n_Aj_Rj[i-1] - n_Rj_S[i-1] - n_Rj_Ekj[i-1] - n_muRj[i-1] - 
  n_Rj_Aj[i-1] else Rj[i] + n_Aj_Rj[i] - n_Rj_S[i] - n_Rj_Ekj[i] - n_muRj[i] - 
  n_Rj_Aj[i]   

# Strain 2 infected(non-GII.4) with strain 1 protection
update(Ekj[]) <- if ( ((step %% 365) == 0) && i > 1 )
  Ekj[i-1] + n_Rj_Ekj[i-1] - n_Ekj_Ikj[i-1] - n_muEkj[i-1] else
  Ekj[i] + n_Rj_Ekj[i] - n_Ekj_Ikj[i] - n_muEkj[i] 

update(Ikj[]) <- if ( ((step %% 365) == 0) && i > 1 )
  Ikj[i-1] + n_Ekj_Ikj[i-1] - n_Ikj_Akj[i-1] - n_muIkj[i-1]  else
  Ikj[i] + n_Ekj_Ikj[i] - n_Ikj_Akj[i] - n_muIkj[i]

update(Akj[]) <-if ( ((step %% 365) == 0) && i > 1 )
  Akj[i-1] + n_Ikj_Akj[i-1] - n_Akj_Rjk[i-1]  - n_muAkj[i-1] else
  Akj[i] + n_Ikj_Akj[i] - n_Akj_Rjk[i]  - n_muAkj[i] 

# Strain 1 infected (GII.4) with strain 2 protection
update(Ejk[]) <- if ( ((step %% 365) == 0) && i > 1 )
  Ejk[i-1] + n_Rk_Ejk[i-1] - n_Ejk_Ijk[i-1] - n_muEjk[i-1] else
  Ejk[i] + n_Rk_Ejk[i] - n_Ejk_Ijk[i] - n_muEjk[i]

update(Ijk[]) <- if ( ((step %% 365) == 0) && i > 1 )
  Ijk[i-1] + n_Ejk_Ijk[i-1] - n_Ijk_Ajk[i-1] - n_muIjk[i-1]  else
  Ijk[i] + n_Ejk_Ijk[i] - n_Ijk_Ajk[i] - n_muIjk[i]

update(Ajk[]) <- if ( ((step %% 365) == 0) && i > 1 )
  Ajk[i-1] + n_Ijk_Ajk[i-1] - n_Ajk_Rjk[i-1]  - n_muAjk[i-1] else
  Ajk[i] + n_Ijk_Ajk[i] - n_Ajk_Rjk[i]  - n_muAjk[i] 

# Recovered and transiently protected from all strains
update(Rjk[]) <- if ( ((step %% 365) == 0) && i > 1 )
  Rjk[i-1] + n_Ajk_Rjk[i-1] + n_Akj_Rjk[i-1] - n_Rjk_S[i-1] - n_muRjk[i-1] else
  Rjk[i] + n_Ajk_Rjk[i] + n_Akj_Rjk[i] - n_Rjk_S[i] - n_muRjk[i]

# Strain 2 infected (non-GII.4) 
update(Ek[]) <- if ( ((step %% 365) == 0) && i > 1 )
  Ek[i-1] + n_S_Ek[i-1] - n_Ek_Ik[i-1] - n_muEk[i-1] else
  Ek[i] + n_S_Ek[i] - n_Ek_Ik[i] - n_muEk[i]

update(Ik[]) <- if ( ((step %% 365) == 0) && i > 1 )
  Ik[i-1] + n_Ek_Ik[i-1] - n_Ik_Ak[i-1] - n_muIk[i-1] else
  Ik[i] + n_Ek_Ik[i] - n_Ik_Ak[i] - n_muIk[i]

update(Ak[]) <- if ( ((step %% 365) == 0) && i > 1 )
  Ak[i-1] + n_Ik_Ak[i-1] + n_Rk_Ak[i-1] - n_Ak_Rk[i-1]  - n_muAk[i-1] else
  Ak[i] + n_Ik_Ak[i] + n_Rk_Ak[i] - n_Ak_Rk[i]  - n_muAk[i]

update(Rk[]) <- if ( ((step %% 365) == 0) && i > 1 )
  Rk[i-1] + n_Ak_Rk[i-1] - n_Rk_S[i-1] - n_Rk_Ejk[i-1] - n_muRk[i-1] - 
  n_Rk_Ak[i-1] else  Rk[i] + n_Ak_Rk[i] - n_Rk_S[i] - n_Rk_Ejk[i] - n_muRk[i] - 
  n_Rk_Ak[i]
...
update(cases_year_str1[]) <- if (step %% steps_per_year==0)
  (if(i==1)  sum(n_Ej_Ij[1]) + sum(n_Ejk_Ijk[1])  else
    if(i==2)  sum(n_Ej_Ij[2:4]) + sum(n_Ejk_Ijk[2:4])  else 
      if(i==3)  sum(n_Ej_Ij[5:14]) + sum(n_Ejk_Ijk[5:14])  else 
        if (i==4) sum(n_Ej_Ij[15:64]) + sum(n_Ejk_Ijk[15:64])  else
          sum(n_Ej_Ij[65:N_age]) + sum(n_Ejk_Ijk[65:N_age]) ) else
            (if(i==1)  cases_year_str1[1] + sum(n_Ej_Ij[1]) + sum(n_Ejk_Ijk[1]) else
              if(i==2)  cases_year_str1[2] + sum(n_Ej_Ij[2:4]) + sum(n_Ejk_Ijk[2:4]) else
                if(i==3) cases_year_str1[3] + sum(n_Ej_Ij[5:14]) + sum(n_Ejk_Ijk[5:14])  else 
                  if (i==4)  cases_year_str1[4] + sum(n_Ej_Ij[15:64]) + sum(n_Ejk_Ijk[15:64])  else
                    cases_year_str1[5] + sum(n_Ej_Ij[65:N_age]) + sum(n_Ejk_Ijk[65:N_age]) )
...
update(cases_year_str2[]) <- if (step %% steps_per_year==0)
  (if(i==1)  sum(n_Ek_Ik[1]) + sum(n_Ekj_Ikj[1])  else
    if(i==2)  sum(n_Ek_Ik[2:4]) + sum(n_Ekj_Ikj[2:4])  else 
      if(i==3)  sum(n_Ek_Ik[5:14]) + sum(n_Ekj_Ikj[5:14])  else 
        if (i==4) sum(n_Ek_Ik[15:64]) + sum(n_Ekj_Ikj[15:64])  else
          sum(n_Ek_Ik[65:N_age]) + sum(n_Ekj_Ikj[65:N_age]) ) else
            (if(i==1)  cases_year_str2[1] + sum(n_Ek_Ik[1]) + sum(n_Ekj_Ikj[1]) else
              if(i==2)  cases_year_str2[2] + sum(n_Ek_Ik[2:4]) + sum(n_Ekj_Ikj[2:4]) else
                if(i==3) cases_year_str2[3] + sum(n_Ek_Ik[5:14]) + sum(n_Ekj_Ikj[5:14])  else 
                  if (i==4)  cases_year_str2[4] + sum(n_Ek_Ik[15:64]) + sum(n_Ekj_Ikj[15:64])  else
                    cases_year_str2[5] + sum(n_Ek_Ik[65:N_age]) + sum(n_Ekj_Ikj[65:N_age]) )
...
update(person_year[]) <- if (step %% steps_per_year==0)
  (if(i==1)  sum(Nrisk[1]) else
    if(i==2)  sum(Nrisk[2:4])  else 
      if(i==3)  sum(Nrisk[5:14])  else 
        if (i==4)  sum(Nrisk[15:64])  else
          sum(Nrisk[65:N_age]) ) else
            (if(i==1)  person_year[1] + sum(Nrisk[1]) else
              if(i==2) person_year[2] + sum(Nrisk[2:4])  else 
                if(i==3) person_year[3] + sum(Nrisk[5:14])   else 
                  if (i==4)  person_year[4] + sum(Nrisk[15:64])  else
                    person_year[5] + sum(Nrisk[65:N_age]) )
